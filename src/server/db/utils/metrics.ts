...
import { eq, and, sql, gte, lte, exists } from 'drizzle-orm'
import { messagesTable, threadsTable } from '~/server/db/schema'
  ...
  const threadMetrics = await db
    .select({
      ...
      resolvedThreads: sql<number>`count(distinct case 
        when ${threadsTable.status} = 'resolved' 
  ...
  const avgFirstResponseTime =
    responseTimeMetrics.reduce((sum, thread) => {
      if (thread.firstCustomerMessage && thread.firstAgentResponse) {
        const responseTime =
          thread.firstAgentResponse.getTime() - thread.firstCustomerMessage.getTime()
